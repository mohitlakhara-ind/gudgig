...
// API Routes - Simplified for gig bidding platform
app.use(etag);
...
io.on('connection', (socket) => {
  ...
  socket.on('gig:newBid', (data) => {
    // Notify gig owner about new bid
    if (data.gigOwnerId) {
      socket.to(`user:${data.gigOwnerId}`).emit('gig:newBid', data);
    }
  });
...
if (process.env.NODE_ENV !== 'test') {
  server.listen(PORT, () => {
    logger.info('server_started', {
      message: `
🚀 Gig Portal Backend Server is running!
📍 Port: ${PORT}
🌍 Environment: ${process.env.NODE_ENV || 'development'}
